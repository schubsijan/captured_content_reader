...
class ArticleNoteService {
  ...
  ArticleNoteService(this.articleId, this.db);
  ...
  Future<void> updateNote(String id, String content, {String? title}) async {
    final notes = await loadNotes();
    final idx = notes.indexWhere((n) => n.id == id);
    if (idx != -1) {
      notes[idx] = notes[idx].copyWith(content: content);
      await _save(notes);
    }
  }
  ...
  Future<void> _save(List<ArticleNote> notes) async {
    ...
    await db.syncNotes(articleId, notes);
  }
}
